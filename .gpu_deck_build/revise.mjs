...
});
{
 ...
 txt(s,'调研修订版  ·  2026.09.14  ·  结论按具体权重、软件栈与硬件条件限定',64,660,1150,25,17,'#A9BDCE');
...
{
 const s=header(5,'2.1  模型适配结论总表','第二部分  /  模型适配结论','“官方已验证”“可进入验证”“当前原版不适配”采用不同证据标准');
 const t=table(s,[['硬件 / 模型','结论','决定结论的证据或缺口'],['双机 910B · GLM-5.3','双机未确认','W8A8C8 官方参考 4 台 A2；双机需新验证'],['双机 910B · DeepSeek-V4-Pro','不满足官方配方','官方 W4A8 已要求至少 4 台 A2'],['双机 910B · Qwen3.8-Flash-Next','有适配，拓扑待核','FlagOS 有昇腾实现；未证明双机 8 卡 910B'],['T4 · Qwen3.8-27B','可进入 PoC','FP16 + PyTorch 回退；数值与逐卡峰值待测'],['T4 · YOLO26','官方 T4 已验证','官方提供 T4 TensorRT 10 检测测试'],['T4 · LTX-2.5 / MAGI-2','不作生产承诺','前者缺 T4 完整证据；后者要求 Hopper']],60,205,1160,400,[409,229,522],23);
 for(let r=1;r<=6;r++)colorCell(t,r,1,r===5?C.teal:(r===2||r===6?C.red:C.amber));
 cite(s,'范围：910B 按 64GB/卡；均未在本项目租赁机实测。Qwen 云端 Flash 对应的公开权重为 Flash-Next');
 notes(s,'本报告没有把资料检索等同于实机成功。双机910B的三个指定超大模型目前不能全部承诺。GLM参考配置不等于严格理论下限；DeepSeek教程明确至少4A2，是当前参考路径的硬要求。Flash-Next存在昇腾定制适配，但发布页单机16设备不能直接对应双机8卡910B。YOLO官方同类T4测试是最强硬件证据，交付机仍需复验。PoC表示有实施路径但尚未全部满足验收条件。',['ag','ad','flag','q27','qcode','yolo','ltx','magi']);
}
{
 const s=header(6,'2.2  双机 910B：精度、框架与 kernel','第二部分  /  模型适配结论','量化必须有对应权重格式和执行实现；更低比特不自动等于可部署');
 table(s,[['模型','已查到的执行路径','两台 8 卡 910B 的判断'],['GLM-5.3\n约 753B','BF16 约 1,506GB，双机装不下\nAscend W8A8C8；vLLM-Ascend\n官方参考 4 A2，标为实验性','W8 理想权重约 753GB\n容量可能，但运行余量 / 拓扑待证\n未找到双机 W4 完整配方'],['DeepSeek-V4-Pro\n约 1.6T / 激活 49B','原权重 FP4 / FP8 不等于 INT4\nAscend 专用 W4A8 + 对应算子\n官方要求至少 4 A2 或 2 A3','理想全 4bit 已约 800GB\n且另有非量化模块与运行开销\n两台 A2 不满足该配方'],['Qwen3.8-Flash-Next\n完整约 180B 起','BF16 仓库 + 定制 SGLang\nFlagGems / CANN / NPU GMM\n示例运行时 MoE 启用 W8A16','BF16 粗估约 360GB 起\n示例单机 16 设备，TP4 / DP4\n缺双机 910B 同拓扑验证']],60,207,1160,374,[263,466,431],23);
 txt(s,'双机方案可先做 Flash-Next 验证；GLM 与 DeepSeek 需供应商提供同配置运行证据。',62,588,1155,40,24,C.teal,true);
 cite(s,'来源：官方模型卡、vLLM-Ascend GLM5.3 / DeepSeek-V4-Pro 教程、FlagRelease 昇腾适配卡');
 notes(s,'权重估算以十进制GB计，不含量化元数据、未量化模块、缓存、工作区和通信。MoE激活参数不能替代驻留总参数。GLM量化从BF16源重新生成，不应把原始FP8权重当作Ascend W8A8C8。vLLM-Ascend配方基于v0.23.0，不能仅升级latest保证兼容。DeepSeek-V4-Pro的W4A8配方已是低精度，A3每台8×128GB与A2每台8×64GB不可互换。FlagRelease发布环境为openEuler22.03 SP4、CANN8.5、torch_npu2.8、SGLang0.5.11、FlagGems5.3.0；还替换qwen4及专用算子文件，映射davinci0..15且TP4 DP4。原始BF16权重仓库不表示所有运行算子都是BF16。该页未明确验证芯片为910B或双机拓扑，不以昇腾品牌名称推定兼容。',['glm','ag','ds','ad','qflash','flag']);
}
{
...
{
 const s=header(8,'2.4  T4 与最新开源视频模型','第二部分  /  模型适配结论','截至调研日，以已公开权重的 LTX-2.5、MAGI-2-preview 为近期代表');
 table(s,[['模型','官方执行条件 / 障碍','T4 决策'],['LTX-2.5','22B 视频模型 + 12B 文本编码器等\nBF16 主路径，低显存路径含 FP8 / 卸载\n有 SDPA 回退，但没有完整 T4 验证','不作为首批交付承诺\nCPU 卸载只缓解容量\nFP16 转换仍需算子与质量验证'],['MAGI-2-preview','114B MoE，激活 6B\n官方要求 8 张 NVIDIA Hopper\n依赖 MagiAttention / MagiCompiler','当前原版路径不适配 T4\n增加 T4 数量不能满足\nHopper 专用执行要求'],['CogVideoX-2B\n较旧的备选','官方推荐 FP16；Diffusers 路径\n模型卡提到量化可在 Colab T4 运行\nCPU 卸载 + VAE 切片 / 分块','可作为 T4 视频 PoC 备选\n不是“最新模型”的替代证明\n时长、画质和速度需实测']],60,208,1160,365,[257,500,403],23);
 txt(s,'若“最新视频模型”是硬性目标，应单独租赁其官方支持的 GPU 配置。',63,586,1154,38,27,C.red,true);
 cite(s,'来源：Lightricks LTX-2.5 / LTX-2；Sand AI MAGI-2-preview；Z.ai CogVideoX-2b 官方模型卡');
 notes(s,'LTX-2.5包含22B transformer、Gemma4-12B文本编码器、视频/音频VAE及上采样器，不能只算22B权重。官方有PyTorch SDPA回退和多GPU方案，因此不能声称必须FlashAttention或完全无多卡支持；但其默认BF16及FP8量化并无完整T4实机证明。MAGI-2官方明确8张Hopper，并非8张任意NVIDIA GPU。CogVideoX-2b官方模型卡推荐FP16，提到torchao/quanto量化使T4可运行，但显存表在A100/H100测得，不能把表中4GB直接当T4保证。选备选时按锁定Diffusers版本验证FP16与INT8后端、CPU卸载、VAE分块；不要使用FP8路径。视频验收需检查完整视频、帧数、分辨率、NaN/黑帧、生成耗时、画质，不用文本Token吞吐评价。',['ltx','ltxcode','magi','cog']);
}
{
...
}
{
 ...
 const links=[['昇腾配方',refs.ad],['Flash-Next 适配',refs.flag],['YOLO / T4',refs.yolo],['视频模型',refs.ltx],['模型下载',refs.hf]];
...
await fs.mkdir(B+'/revision-render',{recursive:true});
await (await PresentationFile.exportPptx(p)).save(B+'/revision-candidate.pptx');
for(let i=0;i<p.slides.items.length;i++){
 const im=await p.export({slide:p.slides.items[i],format:'png',scale:1.5});
 await fs.writeFile(B+'/revision-render/'+String(i+1).padStart(2,'0')+'.png',new Uint8Array(await im.arrayBuffer()));
 console.log('Rendered '+(i+1));
}
await fs.writeFile(B+'/revision-notes.json',JSON.stringify(refs,null,2));
