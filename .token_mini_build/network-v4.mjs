...
const B='/Users/lin/Desktop/catl/.token_mini_build/network-v4';
...
notes(s,'机柜命名按最新反馈：A为6台通算既有机柜，B/C为两台910B及6台T4所在的两个10kW机柜。机柜B含910B-1、T4-1至3，机柜C含910B-2、T4-4至6。业务网和BMC网以交换设备到各代表节点的独立连线展示。通算2…5代表4台分别接入的服务器。存储网明确连接A、B、C三柜，分别代表通算、910B和T4节点按需接入，每柜一条汇总示意线不等于每柜只有一个物理网口，也不代表新增机柜交换机。存储网仍保留可选状态，具体接口、带宽、设备台数、冗余与存储协议待确认。共享存储设备位置未指定。RoCE交换机1台拟放B柜，两台910B各8条200GE链路，16条光链路双端共32个光模块，T4不接入此计算通信组。机柜C至B跨柜布线。单台RoCE交换机不具备交换机级冗余。入口、防火墙、核心及接入交换设备为逻辑职责，复用及采购数量待核。未完成实机连通测试。硬件主题既有机柜字母将在统一编号时与本页保持一致。');}
