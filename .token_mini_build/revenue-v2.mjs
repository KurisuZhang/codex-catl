...
const ROOT='/Users/lin/Desktop/catl',B=ROOT+'/.token_mini_build/revenue-v2';
...
txt(s,'月收入＝每分钟吞吐 × 43,200分钟 × 可计费产出比例 × 对应单价 ÷ 1,000,000',62,202,1156,44,25,C.teal,true);
const vals=[['收入类别','可以直接代入的公式'],
['输出月收入','整套服务输出吞吐 × 43,200 × 可计费产出比例 × 输出单价 ÷ 1,000,000'],
['未命中输入月收入','整套服务未命中输入吞吐 × 43,200 × 可计费产出比例 × 未命中输入单价 ÷ 1,000,000'],
['缓存命中输入月收入','整套服务缓存命中输入吞吐 × 43,200 × 可计费产出比例 × 缓存命中输入单价 ÷ 1,000,000']];
table(s,vals,60,264,1160,[235,925],[35,49,49,49],19);
txt(s,'43,200分钟＝30天 × 24小时 × 60分钟',62,460,1156,29,23,C.navy,true);
txt(s,'可计费产出比例：实际月度可计费用量，占“按该压测速度持续运行30天”理论用量的比例。\n单价使用双方约定的结算价格；三类吞吐已包含参与测试的卡数，不再乘一次卡数。',62,497,1156,61,20,C.ink);
txt(s,'算例：输出 60,000 token/min × 43,200 × 50% × 2元 / 百万 ÷ 1,000,000＝2,592元 / 月',62,577,1156,38,23,C.teal,true);
txt(s,'算例为假设，非实测收益。各类产出比例可不同；已有吞吐中的失败损失不重复扣除，独立峰值不能相加。',62,631,1156,22,17,C.muted);
