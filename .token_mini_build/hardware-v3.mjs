...
const ROOT='/Users/lin/Desktop/catl',B=ROOT+'/.token_mini_build/hardware-v3';
...
for(let k=0;k<3;k++){
 const x=62+k*166, accent=k<2?C.teal:C.blue;
 // Native rack diagram: chassis, mounting rails, server faceplates and empty bays.
 box(s,x+4,249,148,298,'#DCE5EB','#DCE5EB');
 box(s,x,239,152,305,'#263F50','#263F50');
 box(s,x+6,286,140,249,'#142C3C','#142C3C');
 txt(s,k<2?'机柜 '+(k===0?'A':'B'):'机柜 C',x+8,246,136,27,22,C.white,true,'center');
 txt(s,k<2?'10 kW / 拟装载':'既有 / 通算专用',x+4,272,144,22,15,'#CBD9E1',false,'center');
 box(s,x+8,298,7,230,'#6C808D','#6C808D');box(s,x+137,298,7,230,'#6C808D','#6C808D');
 for(let y=305;y<524;y+=14){box(s,x+10,y,3,4,'#192F40','#192F40');box(s,x+139,y,3,4,'#192F40','#192F40');}
 function unit(label,y,h,col){
  box(s,x+18,y,116,h,'#EDF2F5','#A8BAC6');box(s,x+18,y,4,h,col,col);
  box(s,x+26,y+7,3,h-14,'#95A7B3','#95A7B3');
  txt(s,label,x+33,y+1,91,h-2,h>45?18:17,C.navy,false,'center');
  box(s,x+127,y+8,3,3,'#24AF96','#24AF96');
 }
 if(k<2){unit('910B-'+(k+1)+'\n8 张 NPU',303,65,C.teal);for(let j=0;j<3;j++)unit('T4-'+(k*3+j+1)+'  8 卡',374+j*39,34,C.blue);
  for(let j=0;j<3;j++)box(s,x+20,497+j*8,112,3,'#42596A','#42596A');
 }else{for(let j=0;j<6;j++)unit('通算-'+(j+1),303+j*35,30,C.blue);}
 box(s,x+15,544,25,5,'#263F50','#263F50');box(s,x+112,544,25,5,'#263F50','#263F50');
}
