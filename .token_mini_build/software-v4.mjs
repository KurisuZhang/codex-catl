...
const ROOT='/Users/lin/Desktop/catl',B=ROOT+'/.token_mini_build/software-v4';
...
// Execution dependency stack and a separate management lane, not a table.
for(let side=0;side<2;side++){
 const x=62+side*590,col=side===0?1:2,accent=side===0?'#5A8427':'#B63A43',pale=side===0?'#F0F5E8':'#FAEFF0';
 const logo=side===0?'nvidia.png':'huawei.png';
 s.images.add({blob:await fs.readFile(B+'/'+logo),contentType:'image/png',alt:side===0?'NVIDIA 官方 Logo':'华为官方 Logo',fit:'contain',position:{left:x,top:196,width:142,height:43}});
 txt(s,side===0?'T4 软件生态':'910B 软件生态',x+161,199,367,36,26,C.navy,true);
 txt(s,'模型执行路径',x,246,350,25,18,accent,true);
 txt(s,'部署与运维支撑',x+370,246,188,25,18,C.muted,true);
 const rows=[8,7,6,2,1];
 for(let j=0;j<5;j++){
  const y=278+j*62,r=rows[j];
  const sh=box(s,x,y,350,60,pale,'none');sh.borderRadius=5;
  box(s,x,y,4,60,accent,accent);
  txt(s,vals[r][0].replace(/^\d+  /,''),x+13,y+3,325,19,15,accent,true);
  txt(s,vals[r][col],x+13,y+20,325,40,16,C.ink);
  if(j<4){box(s,x+172,y+60,2,2,accent,accent);}
 }
 for(let j=0;j<3;j++){
  const r=[3,4,5][j],y=278+j*104;
  txt(s,vals[r][0].replace(/^\d+  /,''),x+370,y+1,184,26,18,C.navy,true);
  const support=side===0?['containerd / Docker\nNVIDIA Container\nToolkit','NVIDIA Device Plugin\nGFD / GPU Operator\n（统一管理）','DCGM Exporter\nPrometheus / Nsight\n（性能定位）']:['containerd / Docker*\nAscend Docker Runtime\n设备挂载','Ascend Device Plugin\nVolcano / Ascend\nOperator（按需）','NPU Exporter / Prometheus\nNodeD / ClusterD / msprof\n（按需）'];
  txt(s,support[j],x+370,y+27,183,72,15,C.ink);
  box(s,x+370,y+99,183,1,C.line,C.line);
 }
}
box(s,62,601,1148,1,C.line,C.line);
txt(s,'驱动与运行库：设备可用     框架、算子与引擎：模型能跑     部署组件与版本须配套核验',62,610,1156,31,21,C.teal,true);
s.speakerNotes.textFrame.setText('Logo取自NVIDIA官方品牌页及华为官网，仅标识两家技术生态。左侧主栈自底向上展示依赖，侧栏表示部署管理支撑，组件不要求全部安装。原部署影响：OS/CPU/驱动与镜像匹配；运行库ABI、算子与多卡通信；容器访问设备但不转换CUDA；K8s分配设备但不补模型算子；监控用于健康和性能；框架负责模型算子映射；kernel核验硬件/精度/形状；引擎的模型与特性支持取交集。容器、K8s和监控是部署管理面，框架、算子、运行库是模型执行面。各列列出组件范围及替代路径，不要求全部安装。GPU Operator可管理驱动、Container Toolkit、Device Plugin、GFD、DCGM等，不应与另一个驱动生命周期管理方式重复管理。K8s提供调度，不是推理必须条件，首轮可先单容器验通再纳管。NVIDIA宿主机需可用驱动，CUDA用户态库可位于镜像，运行预编译镜像不必把完整CUDA Toolkit一概装在宿主机。Toolkit用于开发编译，不能与Container Toolkit混淆。Ascend容器接入需按运行时、MindCluster和驱动版本选择部署方式，安装工具名称不能证明任意containerd版本兼容。MindCluster中的NodeD、ClusterD、Volcano与Operator按需要配置，非本项目全部必装。NVIDIA T4不假定具备新GPU的所有精度与kernel能力，不照抄MIG方案。跨机NCCL/HCCL还受网卡、RDMA驱动及网络配置影响，双910B沿已定RoCE方案单独验通信。CANN包含运行和开发能力，ATB按引擎需求装。SGLang Ascend依赖包括匹配的Triton-Ascend及SGL Kernel NPU。PD分离还需核验MemFabric等KV传输组件，MoE场景按版本考虑DeepEP兼容实现，这些属于特性依赖，不是所有模型的必装项。MindIE为另一推理路径，不是vLLM的必装前置。SGLang列为候选生态，并未宣称当前默认镜像支持本台T4。YOLO和Wan保持任务专用推理路径，不能因vLLM支持文本就认为其支持任意视觉模型。'+sources([1,2,3,4,5,7,9,11,12]));
}
{
