...
await fs.mkdir(B+'/hierarchy-render',{recursive:true});
await(await PresentationFile.exportPptx(p)).save(B+'/hierarchy-candidate.pptx');
await fs.writeFile(B+'/table-owners.json',JSON.stringify([...new Set(tableOwners)]));
for(let i=0;i<p.slides.items.length;i++){let im=await p.export({slide:p.slides.items[i],format:'png',scale:1});await fs.writeFile(B+'/hierarchy-render/'+String(i+1).padStart(2,'0')+'.png',new Uint8Array(await im.arrayBuffer()));console.log('render '+(i+1));}
