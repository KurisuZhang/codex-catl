...
const ROOT='/Users/lin/Desktop/catl',B=ROOT+'/.token_mini_build/deploy-v3';
...
[['910B 分支','Ascend 镜像＋vLLM Ascend\n按 A2 配方配置精度与并行'],['T4 分支','GGUF＋llama.cpp\nYOLO / Wan 使用独立管线'],['启动与扩展','先单机低负载；双机先验 RoCE\n再配 rank / TP / DP / EP']],
...
txt(s,'执行依据：版本清单＋启动脚本＋测试报告＋平台配置＋回退步骤，详细命令见配套操作清单。',62,617,1156,29,20,C.muted);
s.speakerNotes.textFrame.setText('本页将已确认的12步部署流程整合为四阶段。每阶段三项操作，底部为对应完成标准，勾号表示要求而非已经完成。保留权重下载、量化、分发、环境准备、容器设备透传、推理、扩展、验收、平台接入和故障恢复全部环节。T4与910B是不同的候选执行路径，具体模型以06适配结果为准。单机先验通，双机只在需要时执行，不能将所有模型默认扩至双机。MASS实际字段、镜像digest、模型revision和节点配置尚待现场确认。详细操作和命令见07_模型部署_操作清单.md。\n'+refs);
