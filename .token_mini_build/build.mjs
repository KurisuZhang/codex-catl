...
function header(n,title,sub){let s=p.slides.add();s.background.fill=C.bg;txt(s,title,60,46,1160,70,42,C.navy,true);txt(s,sub,62,126,1156,58,25,C.muted);rule(s,60,659,1160);txt(s,'token 魔方 Mini实验台',60,674,1000,22,15,C.muted);txt(s,String(n).padStart(2,'0')+' / 18',1105,674,115,22,15,C.muted,false,'right');return s;}
...
// 5
{
let s=header(5,'跨柜可共用一台 RoCE 交换机','业务、带外管理与高速计算分网。柜体边界不增加交换跳数。');
let plat=node(s,'目标平台 / 压测入口',62,207,260,55,C.white);let biz=node(s,'业务交换设备',410,207,300,55,'#E9F2FA',C.blue);conn(s,plat,biz,C.blue);
let cpu=node(s,'既有通算资源池 6 台\nMASS 与接入服务',864,207,355,64,C.white);conn(s,biz,cpu,C.blue);
box(s,63,300,543,228,'none');box(s,653,300,567,228,'none');txt(s,'A 柜',77,308,65,32,22,C.muted,true);txt(s,'B 柜',667,308,65,32,22,C.muted,true);
let a=node(s,'910B ×1\n8 张 NPU',84,359,166,70,C.pale,C.teal);let sw=node(s,'RoCE 交换机 ×1\n≥16 个 200GE 接口',326,360,260,70,C.pale,C.teal,22);let b=node(s,'910B ×1\n8 张 NPU',678,360,166,70,C.pale,C.teal);
conn(s,a,sw,C.teal);conn(s,b,sw,C.teal,'left','right');txt(s,'8×200GE',249,329,165,30,20,C.teal);txt(s,'8×200GE 跨柜',580,438,260,30,20,C.teal);
let ta=node(s,'T4 ×3，每台 8 卡',84,473,240,40,C.white,C.blue,22);let tb=node(s,'T4 ×3，每台 8 卡',941,473,258,40,C.white,C.blue,22);
conn(s,biz,a,C.blue,'bottom','top');conn(s,biz,b,C.blue,'bottom','top');
txt(s,'业务网同时接入全部 T4',855,306,345,37,21,C.blue);
let mg=node(s,'运维入口 / 管理交换设备',62,550,350,39,C.white,C.muted,21);let bm=node(s,'全部 14 台服务器 BMC＋交换设备管理口',452,550,767,39,C.white,C.muted,22);conn(s,mg,bm,C.muted);
let sp=node(s,'各资源池存储接口',62,597,350,37,C.white,C.amber,21);let st=node(s,'可选存储网 / 共享存储与备份',452,597,767,37,C.white,C.amber,22);s.shapes.connect(sp,st,{kind:'straight',fromSide:'right',toSide:'left',line:{style:'dashed',fill:C.amber,width:2}});txt(s,'16 条光链路，双端 32 个模块',876,415,330,48,21,C.teal,true);
notes(s,'蓝色为业务网，青色为910B高速计算网，灰色为BMC带外管理。业务网覆盖图示910B与通算，T4业务接入以文字说明。可选存储独立网按模型分发需要启用。跨柜共用同一交换机无新增交换跳数。单RoCE交换机无交换机级冗余。验收：接口与模块兼容、实际距离/光纤/波长/FEC/光功率；16端口速率与映射、MTU/IP、按厂家方案配置PFC/ECN；光误码和丢包检查，再做双机HCCL带宽与实际并行任务。不得仅凭ping。T4不自动接入昇腾HCCL通信组。',['roce','optics']);}
// 6
{
...
await fs.mkdir(B+'/render',{recursive:true});
...
for(let i=0;i<p.slides.items.length;i++){let im=await p.export({slide:p.slides.items[i],format:'png',scale:1});await fs.writeFile(B+'/render/'+String(i+1).padStart(2,'0')+'.png',new Uint8Array(await im.arrayBuffer()));console.log('render '+(i+1));}
