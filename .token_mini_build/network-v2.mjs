...
const B='/Users/lin/Desktop/catl/.token_mini_build/network-v2';
...
box(s,80,399,1120,2,N.mg,N.mg);box(s,90,416,1110,2,N.biz,N.biz);
box(s,259,386,2,13,N.mg,N.mg);box(s,851,386,2,30,N.biz,N.biz);
...
box(s,m.x+18,399,1,32,N.mg,N.mg);box(s,m.x+36,417,1,14,N.biz,N.biz);
...
let stor=node(s,'共享存储 / 对象存储\n存储网（可选）',61,583,343,43,'#E7EFF9',C.ink,19);
// One aggregated dashed connection per rack represents its nodes' storage access.
const spine=box(s,84,554,1110,2,N.storage,N.storage);
box(s,83,554,2,29,N.storage,N.storage);
for(const [x,label] of [[315,'A'],[760,'B'],[1193,'C']]){
 const q=port(x-6,532,13,N.storage);box(s,x,546,1.5,9,N.storage,N.storage);
}
txt(s,'存储网覆盖 A / B / C 三柜节点（按机柜合并示意）',116,558,880,23,17,N.storage);


txt(s,'业务 / BMC / 存储接入线按逻辑汇总；RoCE 保留 16 条物理链路。交换设备数量、速率及冗余另行核定。',62,632,1154,23,17,C.muted);
