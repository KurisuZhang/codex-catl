...
- [21. 采购、机房、交付验收与资产生命周期](#layer-21)
- [22. 安全、隔离与数据治理](#layer-22)
- [23. 平台工程、模型交付与变更管理](#layer-23)
- [24. 知识库治理与团队协作](#layer-24)
...
本知识库将异构验证、训练、可观测性和 SRE 融入主线，并补齐模型下载环节，共包含 25 个模块，编号为 0–24。实际工作不是一次性顺序走完：业务定义验收目标，硬件与软件共同决定容量，压测和成本结果又会反过来改变选型。
...
flowchart TD
    ...
    J --> B
    K --> A
    X[22 安全 / 隔离 / 数据治理] -.贯穿.-> F
    X -.贯穿.-> I
    Y[23 交付工程 + 24 知识治理] -.支撑.-> F
    Y -.支撑.-> G
```

若阅读器不支持 Mermaid，可按“需求 → 工作负载 → 芯片与异构验证 → 整机与采购验收 → 存储 → 模型下载与校验 → 网络与系统 → 平台调度 → 训练/微调 → 模型并行与推理优化 → 可观测性 → MaaS → SRE → 压测 → 经济模型”的顺序阅读。

图中同时表达学习衔接与工程依赖。项目可按业务选择训练自有模型或直接使用已发布模型；采购与机房准备在硬件建设前落实，可观测性、安全和 SRE 则贯穿实施与运行。
...
knowledge-base/
  ...
  model-acquisition/ 模型下载、版本、清单、校验、入库
