...
- [3. GPU 服务器架构](#layer-3)
- [4. 存储与模型分发](#layer-4)
- [5. GPU Fabric、集群网络与集合通信](#layer-5)
- [6. Linux、NUMA 与 PCIe 系统调优](#layer-6)
- [7. GPU Runtime 与容器运行栈](#layer-7)
- [8. Kubernetes GPU 平台](#layer-8)
- [9. GPU 调度与批任务调度](#layer-9)
- [10. 模型架构与分布式并行](#layer-10)
- [11. 推理引擎](#layer-11)
- [12. 推理优化](#layer-12)
- [13. MaaS、Gateway 与多租户服务](#layer-13)
- [14. Benchmark、SLA 验证与容量规划](#layer-14)
- [15. Token 经济模型](#layer-15)
- [三、补充的端到端工程专题](#cross)
- [16. 采购、机房、交付验收与资产生命周期](#layer-16)
- [17. 可观测性与性能诊断](#layer-17)
- [18. 安全、隔离与数据治理](#layer-18)
- [19. SRE、故障处置与灾备](#layer-19)
- [20. 平台工程、模型交付与变更管理](#layer-20)
- [21. 训练、微调与离线任务基础](#layer-21)
- [22. 异构平台与迁移验证](#layer-22)
- [23. 知识库治理与团队协作](#layer-23)
- [四、贯穿案例：从需求到采购数量和 Token 成本](#case)
- [五、12 周新人培养与实操考核](#training)
- [六、可复用工程模板](#templates)
- [七、术语、常见误区与官方资料](#references)
...
原有 0–15 层很好地覆盖了技术主干，但实际工作不是一次性顺序走完：业务定义验收目标，硬件与软件共同决定容量，压测和成本结果又会反过来改变选型。
...
flowchart TD
    ...
    C --> P[16 采购 / 机房 / 到货验收]
    P --> D[4 存储 + 5 网络]
    D --> E[6 Linux + 7 Runtime]
    E --> F[8 Kubernetes + 9 调度]
    F --> G[10 模型并行 + 11 推理引擎]
    G --> H[12 推理优化]
    H --> I[13 MaaS / Gateway]
    I --> J[14 压测 / SLA / 容量]
    ...
    J --> B
    K --> A
    X[17 可观测性 + 18 安全 + 19 SRE] -.贯穿.-> D
    X -.贯穿.-> I
    Y[20 交付工程 + 21 训练 + 22 异构 + 23 知识治理] -.支撑.-> F
    Y -.支撑.-> G
```

若阅读器不支持 Mermaid，可按“需求 → 模型 → 硬件 → 采购验收 → 存储网络 → 系统运行栈 → 平台调度 → 引擎优化 → API 服务 → 压测 → 经济模型”的顺序阅读。

### 1.2 新增知识为何必要

| 补充主题 | 只掌握原技术栈仍无法解决的问题 | 关键交付物 |
| --- | --- | --- |
| 采购与机房 | 卡买到了，但供电、散热、布线或交付条件不成立 | BOM、机房条件表、验收方案、资产台账 |
| 可观测性 | 请求变慢，却不能定位是哪一层消耗了时间 | 指标字典、链路追踪、性能基线 |
| 安全与治理 | 租户能调用 API，但数据和设备边界不清楚 | 权限矩阵、隔离策略、审计与留存方案 |
| SRE 与灾备 | 压测通过，但节点故障、升级和流量突增时不可用 | Runbook、演练记录、恢复目标 |
| 模型与平台交付 | 手工部署可运行，却不能重复发布和可靠回滚 | 版本矩阵、发布流水线、模型清单 |
| 训练与异构 | 只有推理单一路径，无法理解资源竞争和迁移成本 | 作业基线、兼容性与质量评测 |
| 知识治理 | 文档很多，但新人仍靠口头传授 | 学习路径、考核证据、责任人与复查周期 |
...
这些专题贯穿 0–15 层，并非完成 Token 服务以后才开始补做。尤其第 16 章，应在采购前就纳入设计。

<a id="layer-16"></a>
## 16. 采购、机房、交付验收与资产生命周期

### 16.1 从需求到采购合同

采购不是只选择 GPU SKU。需要形成需求基线、RFI/RFQ、完整 BOM、PoC 标准、交付计划、安装责任、验收条件、保修与 RMA 流程。采购、设施、网络、存储、安全、财务和平台团队应明确各自责任。

BOM 至少覆盖整机、CPU、内存、GPU、NIC、系统盘/缓存盘、电源、导轨、线缆、光模块、交换机端口、管理设备、软件许可和备件。替代料必须重新核对性能与兼容性，不能只要求“同等配置”。

合同验收应规定测试方法、环境、数据、允许偏差、失败整改和复验方式。厂商提供的峰值或展示性 Demo 不能代替目标模型的验收结果。

### 16.2 机房工程基础

| 主题 | 需要落实的条件 |
| --- | --- |
| 机柜与空间 | U 位、深度、整机与机柜重量、楼板/运输承载、检修空间 |
| 供电 | 额定输入、配电回路、PDU/插头、电缆、上电浪涌、实际负载与余量 |
| A/B 路冗余 | 一路失效后另一路是否能独立承载所需负载，不能只数双电源 |
| 风冷 | 进风温湿度、冷热通道、风量、背压、热点与持续负载降频 |
| 液冷 | 冷板/CDU、供回水、流量/温差、水质、接口、漏液检测和维护责任 |
| 网络布线 | 端口、长度、介质、极性、标识、走线、备件与故障域 |
| 管理安全 | BMC 管理网、跳板、权限、审计、远程救援路径 |
| 运维支持 | 到场时间、替换件、固件支持周期、整机与部件责任划分 |

机房条件要使用厂商正式安装要求与设施团队核定结果，本文不设跨型号通用的电气和液冷阈值。

### 16.3 分阶段验收

| 阶段 | 验证内容 | 输出 |
| --- | --- | --- |
| 到货核验 | 数量、序列号、SKU、部件、保修、运输损伤 | 到货与差异记录 |
| 静态检查 | 固件、拓扑、链路宽度、传感器、BMC、资产标签 | 配置与拓扑清单 |
| 部件诊断 | CPU/内存、GPU、磁盘、NIC 与链路错误 | 原始诊断报告 |
| 整机持续负载 | 功耗、温度、降频、显存与计算稳定性 | 稳定性基线 |
| 集群测试 | 卡间、多节点、RDMA、存储并发、混合流量 | Fabric 与存储基线 |
| 模型服务 | 目标模型、长短请求、SLO、质量、成本 | 业务验收报告 |
| 失效与恢复 | 重启、部件/链路退出、排空、替换、重新纳管 | 运维验收与复测记录 |

DCGM Diagnostics 可提供运行环境和硬件相关诊断，但测试等级、占用和适用条件不同，不能把一次通过视作永久无故障证明。先在空闲/隔离设备或维护窗口运行有负载的诊断。参考 [DCGM Diagnostics](https://docs.nvidia.com/datacenter/dcgm/latest/user-guide/dcgm-diagnostics.html)。

持续压测时间及各项阈值由风险、交付规模和合同确定；测试通过不代表无需长期错误趋势监控。应保留厂商原始输出与复测结果，避免只有人工写下的“通过”。

### 16.4 生命周期与实践验收

资产状态至少包括在途、待验收、可用、维护、隔离、RMA、退役。记录机柜位置、GPU UUID/序列号、责任人、关联故障及更换历史。退役覆盖权重/数据清理、密钥撤销、盘处理和台账关闭。

新人交付一份可发给采购及设施团队的 BOM 与验收清单，并演练“到货后一张卡长期报错”的隔离、取证、RMA 和恢复流程。验收要求资产、容量、监控与调度状态保持一致。
...
### 17.2 Metrics、Logs、Traces 与 Profile
...
保护的不只是 API Key，还包括模型权重、用户输入、输出、KV/前缀缓存、Adapter、训练数据、Checkpoint、BMC、镜像仓库、集群证书和账单事件。区分“不可信租户能运行代码”和“租户只能调用模型 API”，这两种场景的隔离需求差异很大。

Namespace 是管理边界的一部分，不单独构成强安全隔离；容器、网络、设备和控制面权限都需配合。对不同租户信任等级的方案可参考 [Kubernetes Multi-tenancy](https://kubernetes.io/docs/concepts/security/multi-tenancy/)。

### 18.2 必须掌握

- **身份与密钥**：最小权限、短期凭证、轮换/撤销、Secret 管理、管理面与推理面分离。
- **网络**：入口控制、服务间认证、出网限制、BMC 隔离；RDMA 或额外 NIC 路径应单独测试可达性。
- **主机与容器**：必要设备挂载、最小权限、受控特权组件、镜像扫描与补丁；排障不能长期依赖所有业务容器特权运行。
- **供应链**：镜像/权重/代码哈希、来源、签名或内部验证、依赖锁定、构建记录、SBOM。
- **模型加载**：自定义模型代码可能执行任意代码；`trust_remote_code` 一类开关需要代码审查。安全权重格式也不能替代来源校验和完整应用安全。
- **数据生命周期**：采集授权、分类、脱敏、访问、加密、留存、备份与删除；Prompt/Completion 进入 Trace 前先处理隐私。
- **缓存与设备**：跨租户缓存复用边界、残留状态清理、共享方式与侧信道威胁模型。
- **成本滥用**：超长上下文、无限 Agent 循环、异常并发、慢连接与恶意耗尽预算的限额和审计。
- **应用安全接口**：Prompt Injection、工具执行与检索权限需要模型应用团队处理；Infra 应提供隔离、身份和审计能力，不能宣称 Gateway 已解决语义层攻击。

### 18.3 实践、交付与验收

创建权限矩阵，测试错误租户访问模型/数据、撤销 Key、超长请求、越权网络访问、取消后缓存回收和敏感日志。交付威胁模型与测试证据。模型许可、数据地域和商业使用条件由公司相关负责人依据具体条款审核，不在知识库中把未知条件写成已合规。
...
AI Infra 工程师应能使用 Git、Shell、Python，以及团队主要平台语言；掌握 HTTP/gRPC、流式协议、并发、异常、重试、超时、幂等、结构化日志和基础测试。能读配置、定位调用路径、写可靠的检查工具，比只记住部署命令更重要。

平台侧学习 IaC、配置管理、镜像构建、CI/CD、GitOps、制品仓库、依赖管理和环境分层。裸金属装机、固件/驱动升级、Kubernetes 组件与模型发布应分别有流程，避免一个入口操作同时隐式改动所有层。

### 20.2 模型制品不是一个权重文件

一份可发布模型应包含：模型 ID/Revision、来源与许可记录、权重哈希、Tokenizer、Chat Template、Generation Config、量化/转换配置、Adapter、多模态处理器、引擎兼容信息、质量报告和性能基线。

模型编译或转换产物还需记录目标硬件、工具版本、构建参数和可移植范围。前缀缓存、编译缓存与模型版本耦合，升级后要有明确失效规则。
...
所有环境使用版本化参数和镜像 Digest；禁止用浮动标签解释历史性能。监控阈值、限流策略和价格调整同样属于变更，应记录生效时间与回退方式。

### 20.4 实践、交付与验收

把一次手工模型部署整理成可重复的发布清单与流程，演练回滚到上一版本，并确认模型、Tokenizer、缓存、路由及计量版本一致。交付结果应让另一名新人能在隔离环境复建，不需要依赖作者机器上的未记录状态。

<a id="layer-21"></a>
## 21. 训练、微调与离线任务基础

### 21.1 为什么推理负责人仍需要理解训练
...
每篇工程文档包含负责人、目标读者、适用硬件/版本、最后验证日期、前置知识、具体步骤、预期结果、失败分支、回退方法和原始证据位置。把长期原理、版本操作手册、内部策略和事故复盘分开管理。

文档状态建议使用“草案、已验证、需要复核、已废弃”。链接到 `latest` 的官方资料适合查阅新信息；生产手册应另外锁定版本链接或保存其版本号与检查记录。发生依赖升级、模型更新、重大故障或采购换型后触发复查。

### 23.2 责任划分示例

| 工作 | 主要负责角色 | 必须协作的角色 |
| --- | --- | --- |
| 业务 SLO、质量、优先级 | 产品/业务与模型负责人 | Infra、SRE、成本负责人 |
| GPU 选型与模型基准 | AI Infra | 模型、网络、采购、财务 |
| 机房与上架交付 | 设施/硬件运维 | Infra、网络、供应商 |
| Fabric 与共享存储 | 网络/存储负责人 | Infra、设施、SRE |
| 平台与运行栈 | 平台/AI Infra | 安全、系统运维 |
| 模型发布与性能 | 模型服务/AI Infra | 算法、应用、SRE |
| 数据与权限策略 | 安全/数据治理 | 法务、平台、业务 |
| 用量账单与成本 | 平台计量/财务或 FinOps | Infra、业务 |

小团队可以一人兼多职，但每个关键决策应有明确最终责任人。交接内容不是“找某人问”，而是需求、版本、配置、证据和处置路径。

### 23.3 长期目录建议

以下是后续拆分建议，本次交付仍为这一份完整 Markdown 文件：

```text
knowledge-base/
  overview/          能力地图、术语、阅读路线
  principles/        模型、系统、网络、并行、推理原理
  procurement/       BOM、PoC、机房、验收、资产
  platform/          系统、容器、Kubernetes、调度、异构
  serving/           引擎、优化、Gateway、计量
  reliability/       指标、SLO、Runbook、演练、复盘
  delivery/          制品、版本矩阵、发布、回滚
  economics/         容量、成本、分摊、经营复盘
  labs/              新人实验、样例数据、结果说明
  evidence/          测试报告索引、原始结果位置
```

### 23.4 实践、交付与验收

每名新人完成一个可复现 Runbook、一份性能分析、一篇故障复盘和一次教学分享。由未参与编写的人按文档执行，反馈遗漏后更新。验收以能否复现和解决问题为准，而不是文档页数或缩写数量。

<a id="case"></a>
## 四、贯穿案例：从需求到采购数量和 Token 成本
...
| 2 | 芯片、整机与采购 | 对比两个方案、画服务器拓扑、补齐 BOM | 选型报告、机房条件、验收条款 |
| 3 | Linux 与 Runtime | 复建单节点运行栈、区分几类 OOM | 版本矩阵、只读诊断、复建记录 |
| 4 | 存储与网络 | 冷/热加载及单机通信；有条件做双机 RDMA | 分发和通信基线、瓶颈说明 |
| 5 | Kubernetes GPU | 部署设备组件、配额、探针和租户工作负载 | 分配/隔离测试、Pending 排查 |
| 6 | 调度与并行 | 队列与多卡作业实验，比较两种并行布局 | 排队记录、rank 图、扩展效率 |
...
训练/微调和异构主题可贯穿第 6–12 周，或作为第二阶段专项；不要求 12 周内同时成为网络、Kernel 与训练算法专家。
...
| E03 模型分发 | 同时启动多个副本，测试损坏缓存 | 能定位共享瓶颈并阻止坏权重上线 |
